...
export class MyEventsComponent implements OnInit {
  ...
  fetchEvents() {
    ...
    if (this.tab === 'created') {
      this.eventService.getUserEvents(this.userId).subscribe({
        ...
        error: (err) => { this.error = err.error?.message || 'Failed to load events.'; this.loading = false; }
      });
    } else {
      this.eventService.getEvents().subscribe({
        next: (events) => {
          this.events = events.filter(e => e.attendees?.some(a => a.userId === this.userId && a.status === (this.tab === 'going' ? 'Going' : 'Interested')));
          this.loading = false;
        },
        error: (err) => { this.error = err.error?.message || 'Failed to load events.'; this.loading = false; }
      });
  ...
  cancelEvent(event: Event) {
    ...
    this.eventService.cancelEvent(event.id).subscribe({
      ...
      error: (err) => { this.error = err.error?.message || 'Failed to cancel event.'; }
    });
  }

  reportEvent(event: Event) {
    const reason = prompt('Report reason (Spam, Irrelevant content, Inappropriate/offensive):');
    if (!reason) return;
