...
import {
  ...
  SocialMediaLink,
} from '../expert-applications.service';
...
@Component({
  ...
  template: `
    <app-back-header [title]="'Expert Application'" ></app-back-header>
    <div class="min-h-screen bg-gray-50 py-8">
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
          ...
          <p class="mt-10 text-gray-600" *ngIf="community">
            Submit your application to become an expert in {{ community.title }} community.
          </p>
        </div>
          ...
          <form (ngSubmit)="onSubmit()" #applicationForm="ngForm">
            ...
            <div class="space-y-6 mb-8">
              ...
              <div>
                ...
                <p class="mt-1 text-sm text-gray-500">
                  {{ applicationData.bio.length }}/1000 characters
                </p>
              </div>
            </div>
            ...
            <div class="space-y-6 mb-8">
              ...
              <div class="space-y-4">
                <div
                  *ngFor="
                    let link of applicationData.socialMediaLinks;
                    let i = index
                  "
        ...
        >
          <div
            class="bg-white rounded-lg shadow-lg p-8 max-w-md w-full text-center"
          >
            <div class="text-green-500 text-4xl mb-4">✅</div>
            <h2 class="text-xl font-semibold mb-2">Application Submitted!</h2>
            <p class="text-gray-600 mb-6">
              Your expert application has been submitted successfully. You will
              be notified once it's reviewed by the community administrators.
            </p>
            <button
              (click)="goToCommunity()"
...
export class ExpertApplicationFormComponent implements OnInit {
 ...
 ngOnInit(): void {
  this.authService.currentUser$.pipe(
    ...
    console.log('✅ [expert-form] Final resolved user:', user);

    // ✅ Patch user data to form
    if (user) {
      this.applicationData.name = user.name || '';
      this.applicationData.location = user.location || '';
    } else {
    ...
    console.log('📦 Query param communityId:', communityId);

    if (communityId) {
      this.communityId = communityId;
      this.loadCommunity();
      this.checkExistingApplication(); // If this overrides, handle inside it.
    } else {
      console.warn('❌ No communityId in query params');
    }
  });
}
  ...
  checkExistingApplication() {
    this.expertApplicationService
      .getUserApplication(this.communityId)
      .subscribe({
        next: (application) => {
  ...
  onSubmit() {
    this.errorMessages = this.expertApplicationService.validateApplicationData(
      this.applicationData,
    );
    ...
    if (this.existingApplication?.status === 'pending') {
      ...
      this.expertApplicationService
        .updateApplication(this.existingApplication._id, this.applicationData)
        .subscribe({
          next: (response) => {
          ...
          error: (error) => {
            ...
            this.errorMessages = [
              error.error?.error || 'Failed to update application',
            ];
            this.submitting = false;
          },
        });
      ...
      this.expertApplicationService
        .submitApplication(this.communityId, this.applicationData)
        .subscribe({
          next: (response) => {
            this.showSuccessModal = true;
            this.submitting = false;
